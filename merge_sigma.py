...
def PlotSigmaW(SigmaT, MomGrid, idx, Save=True):
    ...
    ax1.plot(phyFreq, SigmaWp.imag, "bo",
             label="Naive Fourier")
    Nw = len(phyFreq)
    print("C0: ", C0)
    ...
    gw = -phyFreq/(phyFreq**2+((k*Para.kF)**2-Para.kF**2)**2)

    ax1.plot(phyFreq, gw, "go",
             label="Tail $\\sim 1/\omega_n$")
    ax1.plot(phyFreq, gw, "go")
...
if __name__ == "__main__":
    ...
    arr = np.amin(abs(MomGrid-Para.kF))
    kFidx = np.where(abs(arr - abs(MomGrid-Para.kF)) < 1.0e-20)[0][0]
    amax = np.amax(abs(BareGT-BareG))
    print(amax)
    print(np.where(abs(amax - abs(BareGT-BareG)) < 1.0e-20))
