...
int Coeff::floor(double x) const {
  //   cout << "float: " << setprecision(30)
  //        << idx[0] + 1.0 / lambda * log((x - _a) / _b) << endl;
...
int Tau::floor(double x) const {
  // return the index of a given value
  //   cout << "got: " << _coeff0.floor(grid[1]) << endl;
...
vector<double> FermiK::build(double kF, double maxK, int _size, double scale) {
  assert(maxK > kF);
  size = _size;
  assert(size > 2);
  kFidx = size / 2;
...
vector<double> BoseK::build(double kF, double maxK, int _size, double scale) {
  size = _size;
  kFidx = size / 3;
  twokFidx = size / 3 * 2;
  double lambda = kF / scale / kFidx;
...
vector<double> Uniform::build(std::array<double, 2> bounds, int _size) {
  size = _size;
  assert(size > 1);
  grid.resize(size);
  lowerBound = bounds[0];
